use crate::{
    ...
    state::{PoolRewardInfo, Tick, TickUpdate, NUM_REWARDS},
};
...
pub fn next_tick_cross_update(
    ...
    for i in 0..NUM_REWARDS {
        if let Some(reward_info) = reward_infos.get(i) {
        ...
        if let Some(reward_growths_outside) = update.reward_growths_outside.get(i) {
            //  reward_growths_outside =
            let growth_global_x64 = reward_infos
                .get(i)
                .ok_or(ErrorCode::IndexOutOfBounds)?
                .growth_global_x64;
            update.reward_growths_outside[i] =
                growth_global_x64.wrapping_sub(*reward_growths_outside);
        }
    }
    Ok(update)
}
        ...
        if tick.liquidity_gross == 0 {
            ...
            if tick_current_index >= tick_index {
                (
                    fee_growth_global_a,
                    fee_growth_global_b,
                    PoolRewardInfo::to_reward_growths(reward_infos),
                )
            } else {
                (0, 0, [0; NUM_REWARDS])
            }
    ...
    for i in 0..NUM_REWARDS {
        if let Some(reward_info) = reward_infos.get(i) {
            ...
            let reward_growths_below = if !tick_lower.initialized {
                ...
                reward_info
                    .growth_global_x64
                    .wrapping_sub(*tick_lower_reward_growths_outside.get(i).unwrap_or(&0))
            } else {
                *tick_lower_reward_growths_outside.get(i).unwrap_or(&0)
            };
            ...
            let reward_growths_above = if !tick_upper.initialized {
                ...
                *tick_upper_reward_growths_outside.get(i).unwrap_or(&0)
            } else {
                reward_info
                    .growth_global_x64
                    .wrapping_sub(*tick_upper_reward_growths_outside.get(i).unwrap_or(&0))
            };

            reward_growths_inside[i] = reward_info
                .growth_global_x64
