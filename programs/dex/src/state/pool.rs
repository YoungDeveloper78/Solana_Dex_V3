...
// Number of rewards supported by pools
...
impl Pool {
    ...
    pub fn initialize(
        ...
        self.reward_infos =
            [PoolRewardInfo::new(pools_config.reward_emissions_super_authority); NUM_REWARDS];

        Ok(())
    }

    /// Update all reward values for the pool.
    ///
    /// # Parameters
    ...
    /// Update the reward authority at the specified pool reward index.
...
impl PoolRewardInfo {
    ...
    pub fn to_reward_growths(reward_infos: &[PoolRewardInfo; NUM_REWARDS]) -> [u128; NUM_REWARDS] {
...
fn test_pool_reward_info_not_initialized() {
    let reward_info = PoolRewardInfo::default();
    assert_eq!(reward_info.initialized(), false);
}

#[test]
fn test_pool_reward_info_initialized() {
    let reward_info = &mut PoolRewardInfo::default();
...
pub mod pool_builder {
    use super::{Pool, PoolRewardInfo, NUM_REWARDS};
    ...
    impl PoolBuilder {
        pub fn new() -> Self {
            Self {
                reward_infos: [PoolRewardInfo::default(); NUM_REWARDS],
                ..Default::default()
            }
        }
        ...
        pub fn reward_info(mut self, index: usize, reward_info: PoolRewardInfo) -> Self {
            self.reward_infos[index] = reward_info;
            self
        }

        pub fn reward_infos(mut self, reward_infos: [PoolRewardInfo; NUM_REWARDS]) -> Self {
            self.reward_infos = reward_infos;
            self
        }
