...
pub struct CollectRewardV2<'info> {
    ...
    #[account(address = pool.reward_infos[reward_index as usize].mint)]
...
///
/// If the pool reward vault does not have enough tokens, the maximum number of available
...
pub fn handler<'a, 'b, 'c, 'info>(
    ...
    if let Some(reward_info) = position.reward_infos.get(index) {
        let (transfer_amount, updated_amount_owed) = calculate_collect_reward(
            *reward_info, // Use the safely accessed reward_info
...
fn calculate_collect_reward(position_reward: PositionRewardInfo, vault_amount: u64) -> (u64, u64) {
    ...
    let (transfer_amount, updated_amount_owed) = if amount_owed > vault_amount {
        (
            vault_amount,
            amount_owed.checked_sub(vault_amount).unwrap_or(0),
        )
    } else {
        (amount_owed, 0)
    };
