...
describe("whirlpool-impl", () => {
  ...
  const program = anchor.workspace.AnchorDex;
    ...
    describe(`tokenTraitA: ${
      ...
      it("open and add liquidity to a position, trade against it, transfer position to another wallet, then close the tokens to another wallet", async () => {
        ...
        const tokenExtensionCtx = await TokenExtensionUtil.buildTokenExtensionContext(
          ctx.fetcher,
          (await client.getPool(whirlpoolPda.publicKey, IGNORE_CACHE)).getData(),
          IGNORE_CACHE
        );
        ...
        const dWalletTokenAAccount = getAssociatedTokenAddressSync(poolData.tokenMintA, otherWallet.publicKey, undefined, tokenExtensionCtx.tokenMintWithProgramA.tokenProgram);
        const dWalletTokenBAccount = getAssociatedTokenAddressSync(poolData.tokenMintB, otherWallet.publicKey, undefined, tokenExtensionCtx.tokenMintWithProgramB.tokenProgram);
        const rewardAccount0 = getAssociatedTokenAddressSync(poolData.rewardInfos[0].mint, otherWallet.publicKey, undefined, tokenExtensionCtx.rewardTokenMintsWithProgram[0]!.tokenProgram);
        const rewardAccount1 = getAssociatedTokenAddressSync(poolData.rewardInfos[1].mint, otherWallet.publicKey, undefined, tokenExtensionCtx.rewardTokenMintsWithProgram[1]!.tokenProgram);
        const rewardAccount2 = getAssociatedTokenAddressSync(poolData.rewardInfos[2].mint, otherWallet.publicKey, undefined, tokenExtensionCtx.rewardTokenMintsWithProgram[2]!.tokenProgram);
