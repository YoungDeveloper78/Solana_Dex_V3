...
import { InitConfigExtensionParams } from "../../../../src/instructions";
...
describe("initialize_config_extension", () => {
  ...
  async function initializeWhirlpoolsConfigExtension(config: PublicKey, overwrite: Partial<InitConfigExtensionParams>, signers: Keypair[] = [feeAuthorityKeypair]) {
    const pda = PDAUtil.getConfigExtension(ctx.program.programId, config);
    const tx = toTx(ctx, WhirlpoolIx.initializeConfigExtensionIx(ctx.program, {
      feeAuthority: feeAuthorityKeypair.publicKey,
      funder: provider.wallet.publicKey,
  ...
  it("successfully initialize config extension and verify initialized account contents", async () => {
    ...
    const configExtensionPubkey = PDAUtil.getConfigExtension(ctx.program.programId, whirlpoolsConfigKeypair.publicKey).publicKey;
  ...
  it("WhirlpoolsConfigExtension account has reserved space", async () => {
    ...
    const account = await ctx.connection.getAccountInfo(configExtensionPubkey, "confirmed");
    assert.equal(account!.data.length, whirlpoolsConfigExtensionAccountSizeIncludingReserve);
  });
  ...
  describe("invalid input account", () => {
    ...
    it("should be failed: invalid config_extension address", async () => {
      ...
      const invalidPda = PDAUtil.getFeeTier(ctx.program.programId, whirlpoolsConfigKeypair.publicKey, 64);
      await assert.rejects(
        initializeWhirlpoolsConfigExtension(
          whirlpoolsConfigKeypair.publicKey,
          {
    ...
    it("should be failed: funder is not signer", async () => {
      ...
      const whirlpoolsConfigExtensionPda = PDAUtil.getConfigExtension(ctx.program.programId, whirlpoolsConfigKeypair.publicKey);
      const ix: TransactionInstruction = program.instruction.initializeConfigExtension({
        accounts: {
