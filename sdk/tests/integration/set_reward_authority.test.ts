...
import { NUM_REWARDS, toTx, WhirlpoolContext, PoolData, WhirlpoolIx } from "../../src";
...
describe("set_reward_authority", () => {
  ...
  it("successfully set_reward_authority at every reward index", async () => {
    ...
    const pool = (await fetcher.getPool(poolInitInfo.whirlpoolPda.publicKey)) as PoolData;
    for (let i = 0; i < NUM_REWARDS; i++) {
