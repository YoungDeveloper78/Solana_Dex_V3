  ...
  for (const initParam of initParams) {
    ...
    const pools = await Promise.all(
      initPoolParams.map(async (initPoolParam) => {
        ...
        let tokenVaultA = PublicKey.findProgramAddressSync([Buffer.from("pool_vault"), poolPda.publicKey.toBuffer(),tokenMintA.toBuffer()],ctx.program.programId)[0];
        let tokenVaultB= PublicKey.findProgramAddressSync([Buffer.from("pool_vault"), poolPda.publicKey.toBuffer(),tokenMintB.toBuffer()], ctx.program.programId)[0];
...
/**
 * Initialize a brand new set of WhirlpoolsConfig & AnchorDex account
 * @param client - an instance of whirlpool client containing the program & provider
 * @param initSqrtPrice - the initial sqrt-price for this newly generated pool
...
export async function initTestPoolWithTokens(
  ...
  const { tokenMintA, tokenMintB, pool } = poolInitInfo;
