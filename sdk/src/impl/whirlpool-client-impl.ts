...
import { PoolData } from "../types/public";
import { getTickArrayDataForPosition } from "../utils/builder/position-builder-util";
import { PDAUtil, PoolUtil, PriceMath, TickUtil } from "../utils/public";
import { Position, AnchorDex, WhirlpoolClient } from "../whirlpool-client";
...
export class WhirlpoolClientImpl implements WhirlpoolClient {
  ...
  public async getPool(poolAddress: Address, opts = PREFER_CACHE): Promise<AnchorDex> {
    const account = await this.ctx.fetcher.getPool(poolAddress, opts);
    
    
    if (!account) {
      throw new Error(`Unable to fetch AnchorDex at address at ${poolAddress}`);
    }
  ...
  public async getPools(poolAddresses: Address[], opts = PREFER_CACHE): Promise<AnchorDex[]> {
    const accounts = Array.from(
      (await this.ctx.fetcher.getPools(poolAddresses, opts)).values()
    ).filter((account): account is PoolData => !!account);
  ...
  public async getPosition(positionAddress: Address, opts = PREFER_CACHE): Promise<Position> {
    ...
    if (!whirlAccount) {
      throw new Error(`Unable to fetch AnchorDex for Position at address at ${positionAddress}`);
    }
  ...
  public async createPool(
    ...
    const whirlpoolPda = PDAUtil.getPool(
      this.ctx.program.programId,
      whirlpoolsConfig,
    ...
    const tokenVaultAKeypair = PublicKey.findProgramAddressSync([Buffer.from("pool_vault"), whirlpoolPda.publicKey.toBuffer(),new PublicKey(tokenMintA.toString()).toBuffer()], this.ctx.program.programId)[0];
    const tokenVaultBKeypair = PublicKey.findProgramAddressSync([Buffer.from("pool_vault"), whirlpoolPda.publicKey.toBuffer(),new PublicKey(tokenMintB.toString()).toBuffer()], this.ctx.program.programId)[0];
    ...
    const initPoolIx = !TokenExtensionUtil.isV2IxRequiredPool(tokenExtensionCtx)
      ? WhirlpoolIx.initializePoolIx(this.ctx.program, baseParams as any)
