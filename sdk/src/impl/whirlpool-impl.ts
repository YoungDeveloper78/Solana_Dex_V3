...
import { TokenAccountInfo, TokenInfo, PoolData, WhirlpoolRewardInfo } from "../types/public";
...
export class WhirlpoolImpl implements AnchorDex {
  ...
  async getOpenPositionWithOptMetadataTx(
    ...
    if (!whirlpool) {
      throw new Error(`AnchorDex not found: ${translateAddress(this.address).toBase58()}`);
    }
  ...
  async getClosePositionIx(
    ...
    invariant(
      ...
      `Position ${positionAddress.toBase58()} is not a position for AnchorDex ${this.address.toBase58()}`
    );
