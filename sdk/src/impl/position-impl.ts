...
import { PositionData, TickArrayData, TickData, PoolData } from "../types/public";
...
export class PositionImpl implements Position {
  ...
  private whirlpoolData: PoolData;
  ...
  async increaseLiquidity(
    ...
    const whirlpool = await this.ctx.fetcher.getPool(this.data.pool, IGNORE_CACHE);
    if (!whirlpool) {
  ...
  async collectFees(
    ...
    const whirlpool = await this.ctx.fetcher.getPool(this.data.pool, opts);
    if (!whirlpool) {
      throw new Error(
        `Unable to fetch whirlpool (${this.data.pool}) for this position (${this.address}).`
      );
    }
  ...
  private async refresh() {
    ...
    const whirlpoolAccount = await this.ctx.fetcher.getPool(this.data.pool, IGNORE_CACHE);
    if (whirlpoolAccount) {
  ...
  private async updateFeesAndRewards(): Promise<Instruction> {
    const whirlpool = await this.ctx.fetcher.getPool(this.data.pool);
    if (!whirlpool) {
      throw new Error(
        `Unable to fetch whirlpool(${this.data.pool}) for this position(${this.address}).`
      );
